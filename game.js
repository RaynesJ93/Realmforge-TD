...
function renderDungeon(){var g=document.querySelector('#dungeonGrid');if(!g)return;var unlocked=dungeonUnlocked();var frozen=frozenCryptUnlocked();var drowned=drownedTempleUnlocked();var foods=['Cooked Minnow','Cooked Trout','Cooked Pike'];var supply=foods.map(function(n){var q=save.dungeon.supplies[n]||0;var bank=save.bank[n]||0;return '<div class="card"><b>'+n+'</b><div>Bag: '+q+' • Bank: '+bank+'</div><small>Restores '+foodHeal[n]+' lives • Bag max '+(save.hunter.unlocks.supply?10:8)+' total</small><br><button '+(bank<1||supplyCount()>=(save.hunter.unlocks.supply?10:8)?'disabled':'')+' onclick="addDungeonFood(\''+n+'\')">Add</button> <button '+(q<1?'disabled':'')+' onclick="removeDungeonFood(\''+n+'\')">Remove</button> <button '+(!dungeonMode||q<1?'disabled':'')+' onclick="eatDungeonFood(\''+n+'\')">Eat</button></div>'}).join('');var fc=save.dungeon.frozenCrypt,dt=save.dungeon.drownedTemple;g.innerHTML='<div class="card bossCard '+(unlocked?'':'locked')+'"><h3>Greenvale Depths</h3><p>15-wave endurance dungeon. Elite encounters await on waves 5 and 10, with the Root Warden on wave 15.</p><div>Completions: <b>'+save.dungeon.completions+'</b></div><div>Best: <b>'+(save.dungeon.best?(save.dungeon.best/1000).toFixed(1)+'s':'--')+'</b></div><button '+(unlocked?'':'disabled')+' onclick="startDungeon()">'+(unlocked?'Enter Dungeon':'Clear Goblin Stronghold')+'</button></div><div class="card bossCard '+(frozen?'':'locked')+'"><h3>The Frozen Crypt</h3><p>20-wave Frostmere endurance dungeon. Elite encounters strike on waves 5, 10 and 15. Survive wave 20 to face the Crypt King.</p><div>Completions: <b>'+fc.completions+'</b></div><div>Best: <b>'+(fc.best?(fc.best/1000).toFixed(1)+'s':'--')+'</b></div><small>Final Boss: Crypt King • Chest: 1,400–2,000 coins, Frostmere materials, 3 weapons at 3% each and the 1% Crown of the Crypt chase drop.</small><div>Milestones: <b>'+[5,10,25,50,100].map(function(m){return (fc.milestones.includes(m)?'✓ ':'')+m}).join(' • ')+'</b></div><small></small><br><button '+(frozen?'':'disabled')+' onclick="startFrozenCrypt()">'+(frozen?'Enter Frozen Crypt':'Clear Citadel of Ice')+'</button></div><div class="card bossCard '+(drowned?'':'locked')+'"><h3>The Drowned Temple</h3><p>20-wave Blackfen dungeon. Drowned elites attack on waves 5, 10 and 15. The Bog Colossus waits on wave 20.</p><div>Completions: <b>'+dt.completions+'</b></div><div>Best: <b>'+(dt.best?(dt.best/1000).toFixed(1)+'s':'--')+'</b></div><small>Chest: 2,200–3,000 coins, Blackfen materials, 3 dungeon weapons at 3% each and a 1% Bogheart Talisman.</small><div>Milestones: <b>'+[5,10,25,50,100].map(function(m){return (dt.milestones.includes(m)?'✓ ':'')+m}).join(' • ')+'</b></div><br><button '+(drowned?'':'disabled')+' onclick="startDrownedTemple()">'+(drowned?'Enter Drowned Temple':'Defeat the Mire Queen')+'</button></div><h3>Dungeon Supply Bag — '+supplyCount()+'/8</h3><p>Prepare cooked food before entering. Food restores lives only between waves, up to the 20-life maximum.</p><div class="grid">'+supply+'</div>'}

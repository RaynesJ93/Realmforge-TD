...
Object.assign(skillingArtwork,{"Rotwood logs":"assets/skilling/rotwood-logs-v240.webp","Bogiron ore":"assets/skilling/bogiron-ore-v240.webp","Bogiron bar":"assets/skilling/bogiron-bar-v240.webp","Mireweave Cloth":"assets/skilling/mireweave-cloth-v240.webp"});
Object.assign(skillingArtwork,{"Copper ore":"assets/ore-copper-v168.webp","Tin ore":"assets/ore-tin-v168.webp","Ironvale ore":"assets/ore-ironore-v168.webp","Frostsilver ore":"assets/ore-frostsilver-v168.webp","Logs":"assets/wood-logs-v170.webp","Oakheart logs":"assets/wood-oak-v170.webp","Frostpine logs":"assets/wood-frostpine-v170.webp","Goblin scrap":"assets/skilling/goblin-scrap-v241.webp","Golem Core":"assets/skilling/golem-core-v241.webp","Flameguard Sigil":"assets/skilling/flameguard-sigil-v241.webp","Ashweave Cloth":"assets/skilling/ashweave-cloth-v241.webp","Frostweave Cloth":"assets/skilling/frostweave-cloth-v241.webp"});
function menuItemArt(name){const src=skillingArtwork[name]||(itemDB[name]&&window.RealmforgeGear?window.RealmforgeGear.icon(name):null);return src?'<img class="menuItemIcon" src="'+src+'" alt="" width="32" height="32">':'';}
...
const starterEquip=()=>({warrior:{head:'None',body:'Cloth Tunic',weapon:'Rusty Sword'},ranger:{head:'None',body:'Cloth Tunic',weapon:'Old Shortbow'},mage:{head:'None',body:'Cloth Robe',weapon:'Cracked Staff'},summoner:{head:'None',body:'Cloth Robe',weapon:'Cracked Staff'}});
const maps=[
...
{name:'Murkwater Crossing',desc:'Combat Lv 23 • Boglings and Swamp Crawlers infest the drowned road.',mult:5.5,start:275,kinds:['bogling','swampcrawler'],region:'blackfen',req:23},
...
async function recoverOriginalFromCloud(){
 if(!cloudConfigured()){alert('Cloud recovery is unavailable because the cloud backend is not configured. Nothing has been changed.');return}
 if(!cloudSession||!cloudSession.user){alert('Your cloud session is not signed in. Sign in to Realmforge Cloud first, then press RECOVER MY ORIGINAL SAVE again. Nothing has been changed.');return}
 if(cloudBusy){alert('Cloud is already busy. Wait a moment and try recovery again.');return}
 try{
  cloudBusy=true;cloudStatus('Reading cloud recovery save…','working');
  var rows=await cloudRequest('/rest/v1/realmforge_saves?user_id=eq.'+encodeURIComponent(cloudSession.user.id)+'&select=save_data,updated_at&limit=1');
  if(!rows||!rows[0]||!validSave(rows[0].save_data)){throw new Error('No valid Realmforge cloud save was found for this account.')}
  var remote=rows[0].save_data,remoteScore=progressScore(remote),localScore=progressScore(save);
  var remoteClears=remote.campaign&&Array.isArray(remote.campaign.cleared)?remote.campaign.cleared.length:0;
  var localClears=save.campaign&&Array.isArray(save.campaign.cleared)?save.campaign.cleared.length:0;
  if(remoteScore<=localScore&&remoteClears<=localClears){throw new Error('The cloud save does not contain more progress than the current device save, so recovery was stopped safely.')}
  if(!confirm('Cloud recovery found '+remoteClears+' maps cleared and '+(Number(remote.coins)||0).toLocaleString()+' coins. Restore this cloud save to this device?')){cloudStatus('Recovery cancelled — nothing changed','online');return}
  var current=localStorage.getItem(KEY);if(current)localStorage.setItem('realmforge_pre_cloud_recovery',current);
  save=remote;migrateSave();
  localStorage.setItem(KEY,JSON.stringify(save));
  localStorage.setItem(BACKUP_KEY,JSON.stringify(save));
  cloudLastSync=rows[0].updated_at||cloudLastSync;localStorage.setItem('realmforge_cloud_last_sync',cloudLastSync);
  resetBattle();renderUI();cloudStatus('Cloud recovery restored','online');
  alert('Original cloud save restored successfully: '+remoteClears+' maps cleared and '+(Number(save.coins)||0).toLocaleString()+' coins.');
 }catch(e){cloudStatus('Cloud recovery stopped safely','offline');alert('Cloud recovery stopped safely: '+e.message)}
 finally{cloudBusy=false}
}
window.recoverOriginalFromCloud=recoverOriginalFromCloud;
...
function migrateSave(){const skillDefaults=base().skills;save.skills=save.skills||{};Object.entries(skillDefaults).forEach(([name,def])=>{if(!save.skills[name])save.skills[name]={...def};else{save.skills[name].lvl=Math.max(def.lvl,Number(save.skills[name].lvl)||def.lvl);save.skills[name].xp=Math.max(0,Number(save.skills[name].xp)||0)}});save.items=save.items||{};save.drops=save.drops||[];save.bank=save.bank||{};['Copper ore','Tin ore','Bronze bar','Logs','Goblin scrap','Oakheart logs','Ironvale ore','Ironvale bar','Golem Core','Flameguard Sigil','Ashweave Cloth','Raw Minnow','Raw Trout','Raw Pike','Cooked Minnow','Cooked Trout','Cooked Pike','Frostsilver ore','Frostsilver bar','Frostpine logs','Frostweave Cloth','Bogiron ore','Bogiron bar','Rotwood logs','Mireweave Cloth'].forEach(x=>save.bank[x]=Math.max(0,Number(save.bank[x])||0));if(!save.equipment||typeof save.equipment!=='object')save.equipment={};const equipDefaults=starterEquip();['warrior','ranger','mage','summoner'].forEach(function(c){if(!save.equipment[c]||typeof save.equipment[c]!=='object')save.equipment[c]={...equipDefaults[c]};Object.entries(equipDefaults[c]).forEach(function(pair){if(save.equipment[c][pair[0]]===undefined)save.equipment[c][pair[0]]=pair[1]});['offhand','legs','boots','gloves','ring','amulet','cape'].forEach(function(slot){if(save.equipment[c][slot]===undefined)save.equipment[c][slot]='None'})});save.campaign=save.campaign||{unlocked:1,cleared:[],bossKills:0};save.campaign.cleared=save.campaign.cleared||[];save.campaign.unlocked=Math.max(1,save.campaign.unlocked||1);if(save.campaign.cleared.includes(14))save.campaign.unlocked=Math.max(save.campaign.unlocked,16);save.campaign.bossKills=save.campaign.bossKills||0;save.campaign.ashenBossKills=save.campaign.ashenBossKills||0;save.campaign.frostBossKills=save.campaign.frostBossKills||0;save.campaign.mireBossKills=save.campaign.mireBossKills||0;save.collection=save.collection||{};save.bossHunt=save.bossHunt||{best:{warlord:null,tyrant:null},milestones:{warlord:[],tyrant:[]}};save.bossHunt.best=save.bossHunt.best||{warlord:null,tyrant:null};save.bossHunt.best.frostwyrm=save.bossHunt.best.frostwyrm||null;save.bossHunt.best.mirequeen=save.bossHunt.best.mirequeen||null;save.bossHunt.milestones=save.bossHunt.milestones||{warlord:[],tyrant:[]};save.bossHunt.milestones.warlord=save.bossHunt.milestones.warlord||[];save.bossHunt.milestones.tyrant=save.bossHunt.milestones.tyrant||[];save.bossHunt.milestones.frostwyrm=save.bossHunt.milestones.frostwyrm||[];save.bossHunt.milestones.mirequeen=save.bossHunt.milestones.mirequeen||[];save.hunter=save.hunter||{points:0,task:null,unlocks:{elite:false,cache:false,materials:false}};save.hunter.points=Number(save.hunter.points)||0;save.hunter.unlocks=save.hunter.unlocks||{elite:false,cache:false,materials:false};['supply','hunterring','veteran','huntergloves','tracker','master','masteramulet','trophy','huntercape','crest','masterarmour'].forEach(function(k){if(typeof save.hunter.unlocks[k]!=='boolean')save.hunter.unlocks[k]=false});save.quests=save.quests||{started:[],completed:[],kills:{},crafted:{}};save.quests.started=save.quests.started||[];save.quests.completed=save.quests.completed||[];save.quests.kills=save.quests.kills||{};save.quests.crafted=save.quests.crafted||{};save.dungeon=save.dungeon||{completions:0,best:null};save.dungeon.completions=Number(save.dungeon.completions)||0;save.dungeon.best=save.dungeon.best||null;save.dungeon.ashenFalls=save.dungeon.ashenFalls||{completions:0,best:null,milestones:[]};save.dungeon.ashenFalls.completions=Number(save.dungeon.ashenFalls.completions)||0;save.dungeon.ashenFalls.best=save.dungeon.ashenFalls.best||null;save.dungeon.ashenFalls.milestones=save.dungeon.ashenFalls.milestones||[];save.dungeon.frozenCrypt=save.dungeon.frozenCrypt||{completions:0,best:null,milestones:[]};save.dungeon.frozenCrypt.completions=Number(save.dungeon.frozenCrypt.completions)||0;save.dungeon.frozenCrypt.best=save.dungeon.frozenCrypt.best||null;save.dungeon.frozenCrypt.milestones=save.dungeon.frozenCrypt.milestones||[];save.dungeon.drownedTemple=save.dungeon.drownedTemple||{completions:0,best:null,milestones:[]};save.dungeon.drownedTemple.completions=Number(save.dungeon.drownedTemple.completions)||0;save.dungeon.drownedTemple.best=save.dungeon.drownedTemple.best||null;save.dungeon.drownedTemple.milestones=save.dungeon.drownedTemple.milestones||[];save.market=save.market||{merchantDay:'',merchantStock:[]};save.market.merchantDay=save.market.merchantDay||'';save.market.merchantStock=Array.isArray(save.market.merchantStock)?save.market.merchantStock:[];save.market.chestPity=Math.max(0,Math.min(50,Number(save.market.chestPity)||0));save.salvagePoints=Math.max(0,Number(save.salvagePoints)||0);save.enhancements=save.enhancements||{};Object.keys(save.enhancements).forEach(function(n){save.enhancements[n]=Math.max(0,Math.min(5,Number(save.enhancements[n])||0))});save.perks=save.perks||{level:1,xp:0,points:0,unlocked:[],equipped:[],day:'',tasks:[],progress:{}};save.perks.points=Math.max(0,Number(save.perks.points)||0);save.perks.level=Math.max(1,Number(save.perks.level)||1);save.perks.xp=Math.max(0,Number(save.perks.xp)||0);save.perks.unlocked=Array.isArray(save.perks.unlocked)?save.perks.unlocked:[];save.perks.equipped=Array.isArray(save.perks.equipped)?save.perks.equipped:[];save.perks.tasks=Array.isArray(save.perks.tasks)?save.perks.tasks:[];save.perks.progress=save.perks.progress||{};save.perks.completedToday=Array.isArray(save.perks.completedToday)?save.perks.completedToday:[];save.dungeon.supplies=save.dungeon.supplies||{'Cooked Minnow':0,'Cooked Trout':0,'Cooked Pike':0};['Cooked Minnow','Cooked Trout','Cooked Pike'].forEach(function(n){save.dungeon.supplies[n]=Math.max(0,Number(save.dungeon.supplies[n])||0)});if(save.campaign.cleared.includes(4))save.campaign.unlocked=Math.max(save.campaign.unlocked,6);if(save.campaign.cleared.includes(9))save.campaign.unlocked=Math.max(save.campaign.unlocked,11);save.saveVersion=SAVE_VERSION}
...
const realmChests=[
 {id:'supply',name:'Supply Chest',cost:7500,rolls:3,odds:[70,25,5,0,0]},
 {id:'adventurer',name:'Adventurer Chest',cost:30000,rolls:3,odds:[45,35,17,3,0]},
 {id:'heroic',name:'Heroic Chest',cost:90000,rolls:3,odds:[25,35,30,9.5,.5]},
...
Object.assign(itemDB,{
 ...
 'Gilded Longbow':{slot:'weapon',class:'ranger',damage:12,range:34,req:{Ranged:34}},
 'Arcane Warstaff':{slot:'weapon',class:'mage',damage:16,range:22,req:{Magic:34}},
 'Crown of Fortune':{slot:'head',class:'all',damage:4,range:7,defence:14,req:{Defence:30}},
 'Adventurer\'s Ring':{slot:'ring',class:'all',damage:4,range:6,defence:6}
});
function chestProgressTier(){
 var c=save.campaign&&save.campaign.cleared||[];
 return c.includes(14)?3:c.includes(9)?2:c.includes(4)?1:0
}
function chestRewardAllowed(name){
 var it=itemDB[name];if(!it)return true;
 var req=it.req?Math.max.apply(null,Object.values(it.req).map(function(v){return Number(v)||0})):0,t=chestProgressTier();
 return req<=(t===0?12:t===1?24:t===2?36:99)
}
function rollChestRarity(chest){
 var pity=Number(save.market.chestPity)||0;
 if((chest.id==='heroic'||chest.id==='mythic')&&pity>=50)return 'legendary';
 var r=Math.random()*100,sum=0,names=['common','uncommon','rare','epic','legendary'];
 for(var i=0;i<names.length;i++){sum+=chest.odds[i];if(r<sum)return names[i]}
 return 'common'
}
function grantChestReward(rarity){
 var pool=(chestPools[rarity]||chestPools.common).filter(chestRewardAllowed);
 if(!pool.length){rarity='common';pool=chestPools.common.filter(chestRewardAllowed)}
 var name=pool[Math.floor(Math.random()*pool.length)],it=itemDB[name],qty=1;
 if(!it){qty=rarity==='common'?10+Math.floor(Math.random()*16):rarity==='uncommon'?6+Math.floor(Math.random()*10):rarity==='rare'?4+Math.floor(Math.random()*7):2+Math.floor(Math.random()*5);save.bank[name]=(save.bank[name]||0)+qty}
 else save.items[name]=(save.items[name]||0)+1;
 return {name:name,qty:qty,rarity:rarity}
}
function openRealmChest(id){
 var chest=realmChests.find(function(x){return x.id===id});if(!chest)return;
 if(save.coins<chest.cost){alert('You need '+chest.cost.toLocaleString()+' coins to open this chest.');return}
 save.coins-=chest.cost;var rewards=[],legendary=false;
 for(var i=0;i<chest.rolls;i++){var rarity=rollChestRarity(chest),reward=grantChestReward(rarity);rewards.push(reward);if(rarity==='legendary')legendary=true}
...
const enhancementCosts=[0,125,375,875,2000,4500];
...
function bonus(cls,key){const eq=save.equipment&&save.equipment[cls];if(!eq||typeof eq!=='object')return 0;return Object.values(eq).reduce(function(n,name){return n+(itemDB[name]?.[key]||0)+enhancementBonus(name,key)},0)}
...
function bossUnlocked(kind){var mapIndex=kind==='warlord'?4:kind==='tyrant'?9:kind==='frostwyrm'?14:kind==='mirequeen'?19:-1;return mapIndex>=0&&save.campaign.cleared.includes(mapIndex)}
function dungeonUnlocked(){return save.campaign.cleared.includes(4)}
function ashenFallsUnlocked(){return save.campaign.cleared.includes(9)}
...
function startAshenFalls(){if(!ashenFallsUnlocked()){alert('Defeat the Ember Tyrant to unlock The Ashen Falls.');return}bossMode=null;dungeonMode=true;dungeonType='ashenfalls';currentMap=9;bossRunStart=snapshotBossRun();towers=[];enemies=[];shots=[];wave=0;lives=20;battleCoins=215;waveRunning=false;spawnPending=0;mapFinished=false;bossKilled=false;dungeonBossKilled=false;dungeonStart=performance.now();document.querySelectorAll('.screen').forEach(function(x){x.classList.remove('active')});document.querySelector('#battle').classList.add('active');renderUI();hud()}window.startAshenFalls=startAshenFalls;
...
function finishAshenFalls(){
 if(mapFinished)return;mapFinished=true;waveRunning=false;
 var elapsed=Math.round(performance.now()-dungeonStart),af=save.dungeon.ashenFalls;af.completions++;
 if(!af.best||elapsed<af.best)af.best=elapsed;
 [5,10,25,50,100].forEach(function(m){if(af.completions>=m&&!af.milestones.includes(m)){af.milestones.push(m);save.coins+=m*100;save.drops.push('Ashen Falls '+m+' clears milestone: +'+(m*100)+' coins')}});
 var coins=950+Math.floor(Math.random()*451),iron=5+Math.floor(Math.random()*5),cloth=5+Math.floor(Math.random()*5),cores=1+Math.floor(Math.random()*2);
 if(hasPerk('hoarder')){iron=Math.ceil(iron*1.10);cloth=Math.ceil(cloth*1.10)}
...
function renderDungeon(){
 ...
 {key:'ashenfalls',name:'The Ashen Falls',waves:20,boss:'Infernal Warden',elites:'5, 10 and 15',state:save.dungeon.ashenFalls,unlocked:ashenFallsUnlocked(),action:'startAshenFalls()',lock:'Defeat the Ember Tyrant',loot:'950–1,400 coins, Ashen materials, 3 weapons at 3% each and the Infernal Warden Mask at 1%.'},
 ...
 return '<div class="dungeonFood"><div><b>'+menuItemArt(n)+n+'</b><small>Bag '+q+' · Bank '+bank+' · Restores '+foodHeal[n]+' lives</small></div><div class="dungeonFoodActions">'+
...
function bossRewardData(kind,elapsed){
 ...
 return {kind:kind,title:kind==='warlord'?'Goblin Warlord':kind==='tyrant'?'Ember Tyrant':kind==='mirequeen'?'Mire Queen':'Frost Wyrm',rewards:rewards,xp:xp,elapsed:elapsed};
}
...
function showBossResults(kind,elapsed){
 ...
 document.querySelector('#bossResultRewards').innerHTML=d.rewards.length?d.rewards.map(function(x){return '<div class="bossResultRow"><span>'+menuItemArt(x[0])+x[0]+'</span><b>+'+x[1]+'</b></div>'}).join(''):'<div class="bossResultRow"><span>No item/resource drops</span></div>';
 document.querySelector('#bossResultXp').innerHTML=d.xp.length?d.xp.map(function(x){return '<div class="bossResultRow"><span>'+menuItemArt(x[0])+x[0]+' XP</span><b>+'+x[1]+'</b></div>'}).join(''):'<div class="bossResultRow"><span>No XP earned</span></div>';
...
function showMapResults(mapIndex,elapsed){
 ...
 Object.keys(save.items).forEach(function(n){var d=(save.items[n]||0)-(before.items[n]||0);if(d>0)rewards.push([n,d])});
 Object.entries(save.skills).forEach(function(entry){var n=entry[0],v=entry[1],b=before.xp[n];if(!b)return;var d=xpTotalAt(n,v)-xpTotalAt(n,b);if(d>0)xp.push([n,d])});
 var accent=mapIndex<5?'#6f9b55':mapIndex<10?'#d76528':mapIndex<15?'#73b9db':'#8ba553',deep=mapIndex<5?'#1d3524':mapIndex<10?'#401d12':mapIndex<15?'#132f43':'#243a2b';
 ...
 document.querySelector('#bossResultFlavor').textContent=mapIndex<19?'Victory! The next location is ready to be challenged.':'Blackfen conquered!';
 document.querySelector('#bossResultRewards').innerHTML=rewards.length?rewards.map(function(x){return '<div class="bossResultRow"><span>'+menuItemArt(x[0])+x[0]+'</span><b>+'+x[1]+'</b></div>'}).join(''):'<div class="bossResultRow"><span>No additional rewards</span></div>';
...
function startBossHunt(kind){if(!bossUnlocked(kind)){alert('Defeat this boss in the campaign first.');return}bossRunStart=snapshotBossRun();bossMode=kind;currentMap=kind==='warlord'?4:kind==='tyrant'?9:kind==='frostwyrm'?14:19;resetBattle(true);bossMode=kind;bossStart=performance.now();document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));document.querySelector('#battle').classList.add('active');renderUI()}window.startBossHunt=startBossHunt;
...
function renderSmithingTiers(){
 ...
 const openTiers=new Set(Array.from(root.querySelectorAll('details[data-smithing-tier][open]'),d=>d.dataset.smithingTier));
 const grouped=new Map();
 Object.entries(recipes).forEach(entry=>{const key=entry[1].resource;if(!grouped.has(key))grouped.set(key,[]);grouped.get(key).push(entry);});
 const tiers=Array.from(grouped,([resource,entries])=>{entries.sort((a,b)=>a[1].level-b[1].level||a[1].name.localeCompare(b[1].name));return {resource,entries,level:entries[0][1].level};}).sort((a,b)=>a.level-b.level||a.resource.localeCompare(b.resource));
...
const bossArtVersions={warlord:179,tyrant:226,frostwyrm:227,mirequeen:220};
...
function updateSummonerBattleStats(){
 ...
 const st=summonStats(selectedSummon);box.style.display='block';box.innerHTML='<b>'+count+' Summoner'+(count===1?'':'s')+'</b> • '+st.name+' • DMG '+st.damage.toFixed(1)+' • '+(st.rate/1000).toFixed(2)+'s • Range '+st.leash
...
function chooseSummon(kind){
 const d=summonDB[kind],level=(save.skills.Summoning&&save.skills.Summoning.lvl)||1;if(!d)return;if(level<d.level){alert(d.name+' requires Summoning level '+d.level+'.');const s=document.querySelector('#summonSelect');if(s)s.value=selectedSummon;return}
 selectedSummon=kind;const s=document.querySelector('#summonSelect');if(s)s.value=kind;updateSummonerBattleStats()
}
function refreshSummonPicker(){const level=(save.skills.Summoning&&save.skills.Summoning.lvl)||1,s=document.querySelector('#summonSelect');if(s){Array.from(s.options).forEach(o=>{const d=summonDB[o.value];o.disabled=!!d&&level<d.level});s.value=selectedSummon}const hint=document.querySelector('#summonCostHint'),d=summonDB[selectedSummon]||summonDB.spiritwolf;if(hint)hint.textContent='Total: '+(types.summoner.cost+d.cost)}
...
function startTowerAttack(t, target) {
  const duration = t.type === 'warrior' ? .32 : t.type==='summoner' ? .42 : .28;
  t.attackVisual = {life:duration,duration,angle:Math.atan2(target.y-(t.y-30),target.x-t.x)};
  const x=t.x+(t.type==='mage'?-13:t.type==='ranger'?10:t.type==='summoner'?0:-15), y=t.y-(t.type==='mage'?53:t.type==='summoner'?36:42);
...
function drawAttackEffect(s) {
  ...
  if(s.type==='warrior') {
    ...
    if(s.type==='summoner'){ctx.globalAlpha=.9;ctx.fillStyle='#9de7d0';ctx.beginPath();ctx.arc(0,0,11,0,Math.PI*2);ctx.fill();ctx.fillStyle='#d9fff4';ctx.beginPath();ctx.moveTo(7,-8);ctx.lineTo(17,-3);ctx.lineTo(8,1);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(-5,-8);ctx.lineTo(-10,-17);ctx.lineTo(0,-11);ctx.closePath();ctx.fill();ctx.fillStyle='#183c38';ctx.beginPath();ctx.arc(4,-3,2,0,Math.PI*2);ctx.fill();} else if(s.type==='ranger') {
    ...
    ctx.strokeStyle=s.type==='mage'?'#72c4ff':s.type==='summoner'?'#8ff1d2':'#e8dfb4';ctx.lineWidth=2;
...
function drawTowerCharacter(t) {
  if(window.RealmforgeGear&&t.type!=='summoner'){window.RealmforgeGear.drawTower(t);return;}
  ...
  ctx.beginPath();
  ctx.fillStyle = t.type === 'warrior' ? '#777' : t.type === 'ranger' ? '#385f35' : t.type==='summoner' ? '#3d7668' : '#3455a0';
  ctx.arc(t.x, t.y, 18, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = 'white'; ctx.font = '12px sans-serif'; ctx.textAlign = 'center';
  if(t.type==='summoner'){ctx.strokeStyle='#a7ffe4';ctx.lineWidth=3;ctx.beginPath();ctx.arc(t.x,t.y-28,13,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.moveTo(t.x-9,t.y-35);ctx.lineTo(t.x-2,t.y-48);ctx.lineTo(t.x+1,t.y-34);ctx.moveTo(t.x+6,t.y-35);ctx.lineTo(t.x+13,t.y-47);ctx.lineTo(t.x+15,t.y-31);ctx.stroke();ctx.fillStyle='#d7fff3';ctx.fillText('S',t.x,t.y+4);}else ctx.fillText(t.type[0].toUpperCase(), t.x, t.y + 4);
}
function draw(){let m=currentMap;const path=battlePath();ctx.clearRect(0,0,900,520);if(!(window.RealmforgeMaps&&window.RealmforgeMaps.draw(ctx,m,dungeonMode))){ctx.fillStyle=m>=10?'#b7d7df':m>=5?'#5a392f':m===3?'#596451':m===4?'#59633d':'#6f8757';ctx.fillRect(0,0,900,520);ctx.strokeStyle=m>=10?'#dceff2':m>=5?'#8a4c34':m===3?'#625e54':m===4?'#735b42':'#8d7555';ctx.lineWidth=55;ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();path.forEach((p,i)=>i?ctx.lineTo(...p):ctx.moveTo(...p));ctx.stroke();ctx.fillStyle=m===3?'#343a35':'#3c4b31';for(let i=0;i<18;i++)ctx.fillRect((i*137)%880,(i*83)%500,8,8);}drawPlacementGuide();towers.slice().sort((a,b)=>a.y-b.y).forEach(drawTowerCharacter);towers.filter(t=>t.type==='summoner').forEach(function(t){drawSpiritWolf(ensureSpiritWolf(t))});if(towerDrag){ctx.save();ctx.beginPath();ctx.arc(towerDrag.x,towerDrag.y,48,0,Math.PI*2);ctx.fillStyle=towerDrag.valid?'rgba(58,190,82,.18)':'rgba(220,54,54,.20)';ctx.fill();ctx.strokeStyle=towerDrag.valid?'#45e36a':'#ff4747';ctx.lineWidth=7;ctx.stroke();if(!towerDrag.tower){ctx.globalAlpha=.72;drawTowerCharacter({x:towerDrag.x,y:towerDrag.y,type:towerDrag.type,last:0})}ctx.restore()}enemies.slice().sort((a,b)=>a.y-b.y).forEach(e=>{

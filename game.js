...
function ensureSpiritWolf(t){
 ...
 if(!t.summon){const p=nearestPathPoint(t.x,t.y);t.summon={kind:'spiritwolf',x:p.x,y:p.y,last:0,attackUntil:0,targetX:p.x,targetY:p.y}}
 return t.summon
}
function spiritWolfStats(){const level=(save.skills.Summoning&&save.skills.Summoning.lvl)||1;return{range:72,rate:760,damage:9*(1+(level-1)*.035)}}
function updateSpiritWolf(t,now){
 const w=ensureSpiritWolf(t);if(!w)return;
 const anchor=nearestPathPoint(t.x,t.y);w.x=anchor.x;w.y=anchor.y;
 const st=spiritWolfStats(),target=enemies.find(e=>!e.dead&&Math.hypot(e.x-w.x,e.y-w.y)<=st.range);
 if(target&&now-w.last>=st.rate){w.last=now;w.attackUntil=now+180;w.targetX=target.x;w.targetY=target.y;target.hp-=st.damage;addXP('Summoning',2);addXP('Hitpoints',1);if(target.hp<=0&&!target.dead)killEnemyFromTower(target)}
}
function drawSpiritWolf(w){
 ctx.save();const attacking=w.attackUntil>(performance.now()*battleSpeed);ctx.translate(w.x,w.y);ctx.globalAlpha=.92;

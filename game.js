...
function completeCooking(key){if(cookingKey!==key)return;var r=cookDB[key];if((save.bank[r.raw]||0)<1){msg('Cooking stopped: you ran out of '+r.raw+'.');stopCooking();renderUI();return}save.bank[r.raw]--;var chance=Math.max(.02,r.burn-save.skills.Cooking.lvl*.004);if(Math.random()<chance){addXP('Cooking',Math.ceil(r.xp*.25));msg('The fish burns.')}else{save.bank[r.cooked]=(save.bank[r.cooked]||0)+1;addXP('Cooking',r.xp);msg('You cook 1 '+r.cooked+'.')}cookingStart=performance.now();renderUI()}
...
function completeSmithing(key){if(smithingKey!==key)return;let r=recipes[key];if((save.bank[r.resource]||0)<r.bars){msg('Smithing stopped: not enough '+r.resource+'.');stopSmithing();renderUI();return}save.bank[r.resource]-=r.bars;save.items[r.name]=(save.items[r.name]||0)+1;questCraft(r.name);addXP('Smithing',r.xp);smithStart=performance.now();msg('You smith 1 '+r.name+'.');renderUI()}
...
function spawn(kind){let d=enemyDB[kind],scale=dungeonMode?(dungeonType==='frozen'?(1.35+wave*.16):(1.15+wave*.14)):maps[currentMap].mult*(1+wave*.13)*(bossMode?1.08:1);enemies.push({kind,x:path[0][0],y:path[0][1],seg:1,hp:d.hp*scale,max:d.hp*scale,speed:d.speed,reward:Math.round(d.reward*maps[currentMap].mult),dead:false})}
...
function finishMap(){if(mapFinished)return;if(maps[currentMap].boss&&!bossKilled){alert('The boss escaped. This map is not cleared.');return}mapFinished=true;waveRunning=false;if(!save.campaign.cleared.includes(currentMap))save.campaign.cleared.push(currentMap);save.campaign.unlocked=Math.max(save.campaign.unlocked,Math.min(15,currentMap+2));let reward=75+(currentMap*50);save.coins+=reward;if(maps[currentMap].ore)save.bank['Copper ore']+=5;if(maps[currentMap].ashOre)save.bank['Ironvale ore']+=8;if(save.drops.length>20)save.drops=save.drops.slice(-20);persist();if(maps[currentMap].boss&&bossRunStart){var campaignBoss=maps[currentMap].boss;var campaignElapsed=Math.round(performance.now()-bossStart);setTimeout(function(){showBossResults(campaignBoss,campaignElapsed);bossRunStart=null},150)}else{setTimeout(()=>alert(maps[currentMap].name+' cleared! +'+reward+' coins'+(currentMap<14?'\nNext location unlocked.':'\nFrostmere conquered!')),150)}}

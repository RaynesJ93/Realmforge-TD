...
function claimPerkTask(id){var t=perkTaskPool.find(function(x){return x.id===id});if(!t||!save.perks.tasks.includes(id)||(save.perks.progress[id]||0)<t.amount)return;save.perks.tasks=save.perks.tasks.filter(function(x){return x!==id});save.perks.xp+=t.xp;save.perks.points+=(t.points||0);while(save.perks.level<99&&save.perks.xp>=perkXpNeed(save.perks.level)){save.perks.xp-=perkXpNeed(save.perks.level);save.perks.level++}persist()}
const perkShop={pickaxe:{name:'Pickaxe',cost:25,desc:'15% chance when Mining to receive double ore and full XP for both ores.'}};
...
function renderPerks(){
 ...
 stats.innerHTML='<b>Perk Level '+save.perks.level+'</b> • '+save.perks.xp+(need?' / '+need:' MAX')+' XP<br><b>Perk Points: '+save.perks.points+'</b><br>Slots: '+save.perks.equipped.length+' / '+perkSlots();
 g.innerHTML=save.perks.tasks.length?save.perks.tasks.map(function(id){var t=perkTaskPool.find(function(x){return x.id===id}),p=save.perks.progress[id]||0;return '<div class="card"><b>'+t.name+'</b> <small>'+t.tier+'</small><p>'+t.desc+'</p>'+(t.req?'<small>Requirement: '+t.req+' (tasks can roll before you meet it)</small><br>':'')+'<strong>'+p+' / '+t.amount+'</strong><br><small>Reward: '+t.xp+' Perk XP + '+t.points+' Perk Point'+(t.points===1?'':'s')+'</small><br><button '+(p<t.amount?'disabled':'')+' onclick="claimPerkTask(\''+id+'\')">Claim Reward</button></div>'}).join(''):'<div class="card">All five daily tasks completed.</div>';
 var p=perkShop.pickaxe,unlocked=save.perks.unlocked.includes('pickaxe');
 owned.innerHTML='<div class="card"><b>⛏ '+p.name+'</b><p>'+p.desc+'</p><small>'+(unlocked?'Unlocked':p.cost+' Perk Points')+'</small><br>'+(unlocked?'<button onclick="togglePerk(\'pickaxe\')">'+(hasPerk('pickaxe')?'Unequip':'Equip')+'</button>':'<button '+(save.perks.points<p.cost?'disabled':'')+' onclick="buyPerk(\'pickaxe\')">Unlock • '+p.cost+' Points</button>')+'</div>'
}

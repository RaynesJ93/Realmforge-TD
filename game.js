...
async function cloudSignIn(){
 ...
 try{cloudBusy=true;cloudStatus('Signing in…','working');cloudSession=await cloudRequest('/auth/v1/token?grant_type=password',{method:'POST',body:JSON.stringify({email:email,password:password})});localStorage.setItem('realmforge_cloud_session',JSON.stringify(cloudSession));cloudStatus('Cloud connected','online');await cloudPull(false)}catch(e){cloudStatus('Sign in failed','offline');alert(e.message)}finally{cloudBusy=false}
}
...
async function cloudPull(showMessage){
 if(!cloudConfigured()||!cloudSession||!cloudSession.user)return;
 try{cloudBusy=true;cloudStatus('Checking cloud save…','working');var rows=await cloudRequest('/rest/v1/realmforge_saves?user_id=eq.'+encodeURIComponent(cloudSession.user.id)+'&select=save_data,updated_at&limit=1');if(rows&&rows[0]&&validSave(rows[0].save_data)){var remote=rows[0].save_data;if(progressScore(remote)>=progressScore(save)){localStorage.setItem(BACKUP_KEY,JSON.stringify(save));save=remote;migrateSave();writeSave(false);resetBattle();renderUI()}cloudLastSync=rows[0].updated_at||new Date().toISOString();localStorage.setItem('realmforge_cloud_last_sync',cloudLastSync);cloudStatus('Cloud save up to date','online');if(showMessage)alert('Cloud save checked and synced.')}else{cloudBusy=false;await cloudPush(false);if(showMessage)alert('Your current save is now stored in the cloud.')}}catch(e){cloudStatus('Cloud sync failed — local save is safe','offline');if(showMessage)alert(e.message)}finally{cloudBusy=false}
}
function cloudSignOut(){cloudSession=null;localStorage.removeItem('realmforge_cloud_session');cloudStatus(cloudConfigured()?'Signed out':'Cloud backend not connected yet','offline')}
function initCloudSave(){var stored=safeParse(localStorage.getItem('realmforge_cloud_session'));if(stored&&stored.access_token&&stored.user)cloudSession=stored;cloudStatus(!cloudConfigured()?'Cloud backend not connected yet':cloudSession?'Cloud connected':'Signed out',cloudSession&&cloudConfigured()?'online':'offline');if(cloudSession&&cloudConfigured())setTimeout(function(){cloudPull(false)},700)}
window.cloudSignIn=cloudSignIn;window.cloudSignUp=cloudSignUp;window.cloudPush=function(){cloudPush(true)};window.cloudPull=function(){cloudPull(true)};window.cloudSignOut=cloudSignOut;

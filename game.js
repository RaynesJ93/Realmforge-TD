...
function renderSalvage(){
 ...
 g.innerHTML=owned.map(function(n){var qty=salvageAvailable(n),v=salvageValue(n),rare=salvageUniqueNames.includes(n);return '<div class="card marketItem"><b>'+n+'</b><div>'+itemDB[n].class+' • '+itemDB[n].slot+'</div><small>Available: '+qty+' • '+v+' Salvage Points each'+(rare?' • RARE':'')+'</small><div class="marketQuick"><button onclick="salvageItem(\\''+n+'\\',false)">Salvage 1</button><button onclick="salvageItem(\\''+n+'\\',true)">Salvage All</button></div></div>'}).join('')
}
